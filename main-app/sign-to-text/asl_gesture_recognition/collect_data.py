...
#         data_dir = self.setup_data_directory()
...
#         print("💡 Instructions:")
#         print("   - Show the gesture clearly to the camera")
#         print("   - Press 'c' to capture a sample")
#         print("   - Press 'q' to finish early")
#         print("   - Vary your hand position slightly between samples")
#         print("\nStarting in 3 seconds...")
#         time.sleep(3)
        
#         collected = 0
#         auto_capture = False
#         auto_delay = 0.5  # seconds between auto captures
...
#             cv2.putText(frame, "Hand: " + ("✅ Detected" if hand_detected else "❌ Not detected"), 
#                        (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, 
#                        (0, 255, 0) if hand_detected else (0, 0, 255), 2)
#             cv2.putText(frame, "Press 'c' to capture | 'a' for auto | 'q' to quit", (10, 150), 
#                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
#             cv2.putText(frame, f"Auto capture: {'✅ ON' if auto_capture else '❌ OFF'}", (10, 190), 
#                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0) if auto_capture else (0, 0, 255), 2)
...
#             elif (key == ord('c') or auto_capture) and hand_detected:
#                 # Save the image
#                 filename = f"{gesture_name}_{collected:04d}.jpg"
...
#                 if auto_capture:
#                     time.sleep(auto_delay)  # Delay for auto capture
        
#         cap.release()
#         cv2.destroyAllWindows()
        
#         print(f"\n🎉 Finished collecting {collected} samples for '{gesture_name}'")
#         print(f"📁 Data saved in: {gesture_dir}")

# def main():
#     collector = TrainingDataCollector()
    
#     # Common ASL gestures to train
#     common_gestures = [
...
#     print("🤖 ASL Gesture Training Data Collector")
#     print("=" * 50)
    
#     while True:
#         print("\nChoose an option:")
#         print("1. Collect data for common gestures")
#         print("2. Collect data for custom gesture")
#         print("3. View current data collection")
#         print("4. Start training with collected data")
#         print("5. Exit")
...
#             if choice == '1':
#                 print("\nCommon gestures:")
#                 for i, gesture in enumerate(common_gestures[:10], 1):  # Show first 10
#                     print(f"{i}. {gesture}")
#                 print("11. More gestures...")
                
#                 gesture_choice = int(input("Select gesture (1-11): "))
#                 if 1 <= gesture_choice <= 10:
#                     gesture_name = common_gestures[gesture_choice - 1]
#                 elif gesture_choice == 11:
#                     print("\nMore gestures:")
#                     for i, gesture in enumerate(common_gestures[10:20], 11):
#                         print(f"{i}. {gesture}")
#                     print("21. Even more...")
#                     gesture_choice = int(input("Select gesture (11-21): "))
#                     if 11 <= gesture_choice <= 20:
#                         gesture_name = common_gestures[gesture_choice - 1]
#                     else:
#                         print("Using custom gesture...")
#                         gesture_name = input("Enter gesture name: ").strip().replace(' ', '_')
#                 else:
#                     gesture_name = input("Enter gesture name: ").strip().replace(' ', '_')
                
#                 num_samples = int(input("Number of samples (50-200 recommended): "))
#                 collector.collect_gesture_data(gesture_name, min(num_samples, 500))
                
#             elif choice == '2':
#                 gesture_name = input("Enter custom gesture name: ").strip().replace(' ', '_')
#                 num_samples = int(input("Number of samples (50-200 recommended): "))
#                 collector.collect_gesture_data(gesture_name, min(num_samples, 500))
...
#                             samples = len([f for f in os.listdir(gesture_path) if f.endswith('.jpg')])
#                             print(f"  {gesture}: {samples} samples")
#                     else:
...
#                 from simple_training import SimpleGestureTrainer
...
#     # Get gesture name
#     gesture_name = input("Enter gesture name (e.g., 'hello', 'thank_you'): ").strip().replace(' ', '_')
#     num_samples = int(input("Number of samples to collect (50-100 recommended): "))
...
#     existing = len([f for f in os.listdir(gesture_dir) if f.endswith('.jpg')])
...
#         # Display info
#         cv2.putText(frame, f"Gesture: {gesture_name}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
#         cv2.putText(frame, f"Samples: {collected}/{num_samples}", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
#         if hand_detected:
#             cv2.putText(frame, "Hand: ✅ DETECTED - Press 'c'", (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
#             # Draw landmarks
#             mp.solutions.drawing_utils.draw_landmarks(frame, results.multi_hand_landmarks[0], mp_hands.HAND_CONNECTIONS)
#         else:
#             cv2.putText(frame, "Hand: ❌ NOT DETECTED", (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        
#         cv2.imshow('Data Collection', frame)
        
#         key = cv2.waitKey(1) & 0xFF
#         if key == ord('c') and hand_detected:
#             filename = f"{gesture_name}_{existing + collected:03d}.jpg"
#             filepath = os.path.join(gesture_dir, filename)
#             cv2.imwrite(filepath, frame)
#             collected += 1
#             print(f"✅ Captured sample {collected}/{num_samples}")
...
# v2-trying to make it better(works well like terminal based!)
# import cv2
# import mediapipe as mp
# import numpy as np
# import pickle
...
#         """Extract hand landmarks - SAME as your working version"""
#         landmarks = []
...
#     def collect_data(self, gesture_name, num_samples=100, auto_mode=False):
#         """Collect training data with improved UI"""
#         cap = cv2.VideoCapture(0)
#         cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
#         cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
...
#                     if collecting and auto_mode and (current_time - last_capture_time > 0.3):
#                         landmarks = self.extract_landmarks(hand_landmarks)
#                         collected_samples.append(landmarks)
#                         sample_count += 1
#                         last_capture_time = current_time
#                         print(f"✅ Auto-captured sample {sample_count}/{num_samples}")
...
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
#             else:
#                 cv2.putText(frame, "Hand: ❌ NOT DETECTED", (10, 110), 
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            
#             # Mode status
#             if collecting:
#                 mode_text = "AUTO CAPTURING..." if auto_mode else "Press SPACE to capture"
#                 cv2.putText(frame, mode_text, (10, 150), 
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
#             else:
#                 cv2.putText(frame, "Press SPACE to start", (10, 150), 
#                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            
#             cv2.putText(frame, "Press 'q' to finish", (10, 190), 
...
#     def save_dataset(self, new_data, new_labels):
...
#         num_samples = int(input("Number of samples (50-100 recommended): "))
        
#         auto = input("Use auto-capture mode? (y/n): ").strip().lower() == 'y'
        
#         samples = self.collect_data(gesture_name, num_samples, auto_mode=auto)
        
#         if samples:
#             labels = [gesture_name] * len(samples)
#             self.save_dataset(samples, labels)
#         else:
#             print("No samples collected!")
...
#             samples = self.collect_data(gesture, num_samples, auto_mode=auto)
...
#         if all_samples:
#             self.save_dataset(all_samples, all_labels)
#         else:
#             print("No samples collected!")
    
#     def run(self):
#         """Main menu"""
#         print("🤖 ASL Gesture Data Collector (Improved)")
#         print("=" * 50)
...
#                 if confirm == 'yes':
#                     if os.path.exists('data/gesture_dataset.pickle'):
#                         os.remove('data/gesture_dataset.pickle')
